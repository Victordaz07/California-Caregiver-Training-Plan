...
| "18 de 52 episodios descargados" / "420 MB / 1.2 GB" (barra de progreso de descarga) | A-025 — no existía ninguna descarga real; el paquete no incluye audio narrado (ver `DECISIONS.md` ADR-006) | "Los 13 episodios están incluidos en la app con audio narrado real: disponibles sin conexión, sin nada que descargar" | BibliotecaAudiosScreen.kt |
| Botón "Escuchar Audio" sin `onClick` (simulaba reproducción de narración inexistente) | A-024/A-026 — en su momento no había archivos de audio narrado en el paquete de origen | Reemplazado primero por `VoiceRecordCard` (grabación propia); en un lote posterior, un botón de reproducción real (`AudioNarrationController`) sobre los 13 `.mp3` generados con Cloud Text-to-Speech — ver ADR-008 | AudioLeccionesScreen.kt, BibliotecaAudiosScreen.kt (narración real); RolePlayBilingueScreen.kt, SimulacionTurnoScreen.kt (grabación propia, sin narración pregrabada aplicable) |
