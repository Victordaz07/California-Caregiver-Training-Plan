...
Esta sesión cerró la mayoría de los hallazgos regulatorios Críticos y Altos que eran correcciones de **contenido/texto/estructura de navegación** (A-001 a A-020, A-028 a A-039, A-042, A-044 — ver tabla abajo). No cerró, y no podía cerrar en una sola sesión, los hallazgos que requieren **funcionalidad real no implementada todavía**: audio real (A-024/A-025), grabación (A-026/A-041), repetición espaciada (A-023), evidencia de competencias separada (A-040), currículo completo de 90 días (A-022), y ninguna prueba automatizada (A-043) porque este entorno no tiene acceso al Android SDK y no pudo compilar ni ejecutar nada (ver `TEST_REPORT.md`). Por eso el estado no es GO: falta funcionalidad Alta/Crítica real, y nada de lo escrito en esta sesión tiene evidencia de compilación.
...
| A-020 | Muchos botones sin acción | 11 controles documentados explícitamente como pendientes (no ocultos ni fingidos) | Ver `TEST_REPORT.md` | Inventario manual | Parcial — deuda documentada, no oculta |
| A-021 | Sin persistencia | DataStore persiste `pathwayId`/`onboardingCompleted` de verdad | `UserPreferencesRepository.kt` | Revisión de código (DataStore es persistente por diseño) | Parcial — solo ruta, no progreso de aprendizaje (eso requiere Room, ver DECISIONS.md) |
| A-022 | Solo semana de muestra | Sin cambios — currículo completo (90 filas de CSV) no importado | — | — | Pendiente (fuera de alcance de este lote) |
| A-023 | Repetición espaciada solo de etiqueta | Sin cambios — no implementado | — | — | Pendiente |
| A-024/A-025 | Audio/descargas simuladas | Sin cambios — Media3 no añadido | — | — | Pendiente (requiere dependencia no verificable en este entorno) |
| A-026/A-041 | Grabación sin flujo de privacidad | Sin cambios — MediaRecorder no añadido | — | — | Pendiente |
...
|---|---|---|---|---|
| Crítica | Ningún archivo de este lote tiene evidencia de compilación real | Todo el trabajo podría tener errores de tipos/API no detectables sin SDK | Abrir `android/` en Android Studio y sincronizar | Ejecutar `./gradlew :app:assembleDebug` y corregir lo que falle; ver "Riesgos de verificación" en `DECISIONS.md` |
| Alta | A-022: currículo de 90 días no importado (solo semana 1 de ejemplo) | Plan90DiasScreen sigue mostrando datos de muestra | Abrir la pestaña Plan | Importar `CURRICULO_90_DIAS.csv` (92 filas) a un modelo de datos real |
| Alta | A-023/A-024/A-025/A-026: motor de aprendizaje, audio real y grabación no implementados | 10 controles primarios siguen sin acción (ver `TEST_REPORT.md`) | Tocar cualquiera de esos botones | Implementar Fase 4 y Fase 6 del prompt maestro, en una sesión con SDK disponible para verificar Media3/MediaRecorder |
| Alta | A-043: cero pruebas automatizadas | No hay forma de detectar regresiones futuras sin revisión manual | — | Añadir JUnit/Compose UI tests una vez que haya SDK disponible para ejecutarlos |
