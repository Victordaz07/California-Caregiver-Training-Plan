...
| A-024/A-025 | Audio/descargas simuladas | Se descubrió que el paquete no incluye ningún archivo de audio narrado real (ver `DECISIONS.md` ADR-006); no se añadió Media3 ni se simuló reproducción. Las cifras falsas de descarga se retiraron y la biblioteca de 13 guiones se muestra honestamente como ya incluida sin conexión | `content/AudioEpisodes.kt`, `AudioLeccionesScreen.kt`, `BibliotecaAudiosScreen.kt` | Revisión manual + `find` sobre el paquete completo (0 archivos de audio) | Resuelto de forma distinta a la especificación original — sin audio narrado real porque no existe en el paquete de origen; ver ADR-006 |
...
| Alta | A-024/A-025: no existe audio narrado real reproducible (el paquete de origen no incluye ningún archivo de audio) | Las lecciones de audio son guiones de lectura + grabación de la propia voz, no narración profesional pregrabada | Abrir cualquier pantalla de Audio | Grabar/conseguir narración real (fuera del alcance de esta sesión, requiere producción de audio, no código) |
